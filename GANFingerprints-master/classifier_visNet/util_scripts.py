...
def classify(model_path, testing_data_path, out_fingerprint_dir):
    ...
    if not os.path.isdir(out_fingerprint_dir):
        os.mkdir(out_fingerprint_dir)
    ...
    if testing_data_path.endswith('.png') or testing_data_path.endswith('.jpg'):
        ...
        for (count0, name) in enumerate(name_list):
            ...
            array.append(labels[idx])
            l = logits[0].tolist()
            l = [float('%.3f' % (item)) for item in l]
            final_values.append(l)
        ...
        if os.path.isfile('result'):
            ...
            spliter = testing_data_path.split("/")
            key = spliter[len(spliter) - 2]
            result[key] = array
            val_name = key + "values"
            result[val_name] = final_values
